...
class SLIC(object):
    def __init__(self, HSI, labels, class_count, gt, n_segments=1000, compactness=20, max_iter=20, sigma=0, min_size_factor=0.3,
                 max_size_factor=2):
    ...
    def get_Q_and_S_and_Segments(self):
        ...
        S = np.zeros([self.class_count+1, d], dtype=np.float32)
        Q = np.zeros([w * h, self.class_count+1], dtype=np.float32)
        x = np.reshape(img, [-1, d])

        for i in range(self.class_count+1):
            (H, W) = self.gt.shape
            gt_flatten = np.reshape(self.gt, [H*W])
            idx = np.where(gt_flatten == i)[0]
            count = len(idx)
            pixels = x[idx]
    ...
    def get_A(self, sigma: float, S):
        ...
        A = np.matmul(S, S.T)
        # S = torch.tensor(S)
        # a_input = torch.cat([S.repeat(1, N).view(N * N, -1), S.repeat(N, 1)], dim=1).view(N, -1,
        #                                                                                   2 * d)  # [N, N, 2*out_features]
        # a = torch.nn.Parameter(torch.zeros(size=(2 * d, 1)))
        # torch.nn.init.xavier_uniform_(a.data, gain=1.414)
        # leakyrelu = torch.nn.LeakyReLU(0.2)
        # e = leakyrelu(torch.matmul(a_input, a).squeeze(2))  # [N, N]   图注意力的相关系数

        # zero_vec = -9e15 * torch.ones_like(e)  # [N, N]
        # attention = torch.where(adj > 0, e, zero_vec)
        # A = torch.sigmoid(torch.tensor(A)).numpy()
        # B = np.identity(self.class_count+1)
        # A = A - B
        # A = np.zeros([self.class_count+1, self.class_count+1], dtype=np.float32)
        # (h, w) = self.gt.shape
        # for i in range(h - 2):
        #     for j in range(w - 2):
        #         sub = self.gt[i:i + 2, j:j + 2]
...
class LDA_SLIC(object):
    def __init__(self, data, labels, n_component, class_count, gt):
        self.data = data
    ...
    def SLIC_Process(self, img, scale=25):
        ...
        myslic = SLIC(img, n_segments=n_segments_init, labels=self.labes, compactness=1, sigma=1, min_size_factor=0.1,
                      max_size_factor=2, class_count=self.class_count, gt=self.gt)
        # Q, S, Segments = myslic.get_Q_and_S_and_Segments()
        Q, S = myslic.get_Q_and_S_and_Segments()
        A = myslic.get_A(sigma=10, S=S)
    ...
    def simple_superpixel(self, scale):
        ...
        Q, S, A = self.SLIC_Process(X, scale=scale)
        return Q, S, A#, Seg

    def simple_superpixel_no_LDA(self, scale):
        Q, S, A = self.SLIC_Process(self.data, scale=scale)
        return Q, S, A
